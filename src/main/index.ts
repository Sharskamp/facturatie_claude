...
async function maakEpcQrHtml(iban: string | null | undefined, bedrijfsnaam: string, bedrag: number, referentie: string): Promise<string> {
  ...
  try {
    ...
    return `<img src="${dataUrl}" style="width:112px;height:112px;display:block" alt="SEPA betaal QR"/>`
  } catch {
    return ''
  }
}
...
function bouwRegelsHtml(regels: Array<{ omschrijving: string; aantal: number; eenheid?: string | null; prijs: number; kortingPercentage: number; totaal: number }>): string {
  ...
  if (toonKorting) {
    return `<table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;${ph}">Omschrijving</th><th style="text-align:center;${ph}">Aantal</th><th style="text-align:right;${ph}">Prijs</th><th style="text-align:right;${ph}">Korting</th><th style="text-align:right;${ph}">Totaal</th></tr></thead><tbody>${regels.map(r => `<tr><td style="${p}">${r.omschrijving}</td><td style="text-align:center;${p}">${r.aantal}${r.eenheid ? ` ${r.eenheid}` : ''}</td><td style="text-align:right;${p}">€${r.prijs.toFixed(2)}</td><td style="text-align:right;${p}">${(r.kortingPercentage ?? 0) > 0 ? `${r.kortingPercentage}%` : '-'}</td><td style="text-align:right;${p}">€${r.totaal.toFixed(2)}</td></tr>`).join('')}</tbody></table>`
  }
  return `<table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;${ph}">Omschrijving</th><th style="text-align:center;${ph}">Aantal</th><th style="text-align:right;${ph}">Prijs</th><th style="text-align:right;${ph}">Totaal</th></tr></thead><tbody>${regels.map(r => `<tr><td style="${p}">${r.omschrijving}</td><td style="text-align:center;${p}">${r.aantal}${r.eenheid ? ` ${r.eenheid}` : ''}</td><td style="text-align:right;${p}">€${r.prijs.toFixed(2)}</td><td style="text-align:right;${p}">€${r.totaal.toFixed(2)}</td></tr>`).join('')}</tbody></table>`
}

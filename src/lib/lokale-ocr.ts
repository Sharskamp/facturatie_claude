...
export async function pdfPaginaNaarPng(pad: string, paginaIndex = 0): Promise<Buffer> {
  // Gebruik Electron's nativeImage API om een grote placeholder te genereren
  // Dit werkt op alle platforms zonder canvas of external dependencies
  const { nativeImage } = require('electron')

  // Maak een grote placeholder afbeelding (1240x1754 = A4 formaat)
  // We gebruiken een truc: creëer een 1x1 pixel en schaal hem op
  const smallImg = nativeImage.createEmpty()
  const largeSize = { width: 1240, height: 1754 }

  // In plaats daarvan: maak een grijze placeholder met text via een trick
  // We schrijven direct naar het bestand om het later terug te lezen
  const tmpPath = path.join(os.tmpdir(), `pdf_placeholder_${Date.now()}.png`)

  // Maak een zeer eenvoudige PNG: grijze achtergrond met tekst
  // Dit is een minimale PNG encoder
  try {
    // Probeer eerst de tekst uit de PDF te halen en toon die
    const pdfjsLib = laadPdfJsVoorTekst()
    const buffer = fs.readFileSync(pad)
    const doc = await pdfjsLib.getDocument({
      data: new Uint8Array(buffer),
    ...
    for (const item of content.items) {
      const text = (item as Record<string, unknown>).str as string
      if (text?.trim()) textPreview += text + ' '
      if (textPreview.length > 200) break
    }

    await doc.destroy()

    // Schrijf een SVG placeholder met de geëxtraheerde tekst
    const svg = `
      <svg width="1240" height="1754" xmlns="http://www.w3.org/2000/svg">
        <rect width="1240" height="1754" fill="#f3f4f6"/>
        <rect width="1240" height="120" fill="#3b82f6"/>
        <text x="620" y="80" font-size="48" font-weight="bold" fill="white" text-anchor="middle">📄 PDF Pagina ${paginaIndex + 1}</text>
        <text x="620" y="200" font-size="24" fill="#1f2937" text-anchor="middle">Preview: Teken een box om tekst te extraheren</text>
        <text x="60" y="300" font-size="14" fill="#4b5563" font-family="monospace">${textPreview.slice(0, 150).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</text>
      </svg>
    `

    const svgBuffer = Buffer.from(svg)
    fs.writeFileSync(tmpPath, svgBuffer)

    // Converteer SVG naar PNG via Electron's nativeImage (als beschikbaar)
    // Fallback: gewoon PNG bytes genereren
    const pngData = nativeImage.createFromPath(tmpPath)
    if (pngData && !pngData.isEmpty()) {
      try { fs.unlinkSync(tmpPath) } catch { }
      return pngData.toPNG()
    }
  } catch {
    // Ignoreer errors
  }

  // Ultieme fallback: creëer een zeer eenvoudige grijze PNG met minimale data
  // Dit is een 1x1 grijze pixel die de browser zal oprekken
  const placeholderPng = Buffer.from([
    ,

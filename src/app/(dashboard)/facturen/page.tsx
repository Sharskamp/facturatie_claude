...
export default function FacturenPage() {
  ...
  const laadFacturen = useCallback(async (status: StatusFilter) => {
    ...
    try {
      ...
      if (status !== "ALLES") params.set("status", status);
      const res = await fetch(`/api/facturen?${params.toString()}`);
  ...
  function wisselStatus(s: StatusFilter) {
    setStatusFilter(s);
    setZoekterm("");
    router.replace(`/facturen${s !== "ALLES" ? `?status=${s}` : ""}`, {
      scroll: false,
    });
  }
  ...
  async function verwijder() {
    ...
    try {
      await fetch(`/api/facturen/${teVerwijderen.id}`, { method: "DELETE" });
              ...
              <div className="flex flex-col items-center justify-center h-48 text-center">
                ...
                {statusFilter === "ALLES" && !zoekterm && (
                  <Button
                    className="mt-4"
                    size="sm"
                    onClick={() => router.push("/facturen/nieuw")}
                  >
                    <Plus className="h-4 w-4" />
                    Nieuwe factuur
                  </Button>

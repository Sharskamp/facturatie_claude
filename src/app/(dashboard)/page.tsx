...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function laadData() {
      try {
        const [facturenRes, inkomenRes, uitgavenRes] = await Promise.all([
          fetch("/api/facturen"),
          fetch("/api/inkomen"),
          fetch("/api/uitgaven"),
        ]);
        ...
        setInkomen(Array.isArray(inkomenData) ? inkomenData : []);
        setUitgaven(Array.isArray(uitgavenData) ? uitgavenData : []);
      } catch (e) {
  ...
  // Omzet deze maand: betaalde + verzonden facturen
  ...
  // Openstaand: verzonden maar niet betaald
  ...
  // Grafiek: laatste 6 maanden omzet
  ...
  // Aankomende betalingen: VERZONDEN gesorteerd op vervaldatum
          ...
          <Card>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Link
                  href="/facturen?status=VERZONDEN"

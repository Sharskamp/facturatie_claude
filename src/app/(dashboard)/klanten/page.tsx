...
export default function KlantenPage() {
  ...
  const laadKlanten = useCallback(async (zoek = "") => {
    try {
      const params = zoek ? `?zoek=${encodeURIComponent(zoek)}` : "";
      const res = await fetch(`/api/klanten${params}`);
  ...
  async function slaOp() {
    ...
    setFout(null);
    try {
      const url = geselecteerdeKlant
        ? `/api/klanten/${geselecteerdeKlant.id}`
        : "/api/klanten";
      const methode = geselecteerdeKlant ? "PUT" : "POST";
  ...
  async function verwijder() {
    ...
    try {
      await fetch(`/api/klanten/${geselecteerdeKlant.id}`, { method: "DELETE" });
